...
function addRow() {
    ...
    const row = `<tr><td>${rowCount}</td><td>#</td><td>High</td><td>New Incident</td><td>Enter observation...</td><td>Open</td><td>TBD</td></tr>`;
...
function managePagination() {
    ...
    // Move overflowing rows to next page
...
function createNewIncidentPage(afterPage) {
    ...
    newPage.innerHTML = `
        <div style="flex-grow: 0; display: flex; flex-direction: column; position: relative;">
            <h3 class="section-header" style="margin-top:10px; margin-bottom:8px;">${headingNum}. Potential Incidents (Continued)</h3>
            <div class="box-border" style="padding:0; margin-bottom: 40px; border: 2px solid #000; flex-grow: 0; min-height: fit-content; height: auto;">
                <table class="data-table">
                    ...
                    <tbody class="incident-tbody" contenteditable="true">
                    </tbody>
                </table>
            </div>
        </div>
...
function updatePageNumbers() {
    // Only number pages that are visible
    let pages = Array.from(document.querySelectorAll('.page')).filter(p => !p.style.display || p.style.display !== 'none');

    pages.forEach((p, index) => {
        let numEl = p.querySelector('.page-num');
        if (numEl) numEl.innerText = index + 1;
    });

    let bpSection = document.getElementById('bluPineSectionWrapper');
    if (bpSection && bpSection.style.display !== 'none') {
        let pPage = bpSection.closest('.page');
        let index = pages.indexOf(pPage);
        let tocBluPineObj = document.getElementById('tocBluPinePage');
        if (tocBluPineObj && index !== -1) tocBluPineObj.innerText = index + 1;
    }

    let sec5Wrapper = document.getElementById('section5Wrapper');
    if (sec5Wrapper) {
        let pPage = sec5Wrapper.closest('.page');
        let index = pages.indexOf(pPage);
        let tocItem = document.getElementById('tocIncidentPage');
        if (tocItem && index !== -1) tocItem.innerText = index + 1;
    }

    document.querySelectorAll('.end-doc-pin').forEach(pin => pin.remove());
    let lastPage = pages[pages.length - 1];
    if (lastPage) {
        let pin = document.createElement('div');
        pin.className = 'end-doc-pin';
        pin.style.cssText = 'bottom: 80px; width: 100%; text-align: center; margin-top: 40px; margin-bottom: 20px;';
        pin.innerHTML = '<b>← End of Document →</b>';

        let secWrapper = lastPage.querySelector('#section5Wrapper');
        let p7Container = lastPage.querySelector('#p7Container');
        let incContainer = lastPage.querySelector('#incidentSectionsContainer');

        if (secWrapper) {
            secWrapper.appendChild(pin);
        } else if (p7Container) {
            p7Container.appendChild(pin);
        } else if (incContainer) {
            incContainer.appendChild(pin);
        } else {
            // Fallback for pages without the container
            let flexCol = lastPage.querySelector('div[style*="flex-grow: 1"]');
            if (flexCol) {
                flexCol.appendChild(pin);
            } else {
                let pinContainer = lastPage.querySelector('.footer-text-row');
                if (pinContainer) pinContainer.parentNode.insertBefore(pin, pinContainer);
            }
        }
    }
}
    ...
    Papa.parse(e.target.files[0], {
        ...
        complete: function (results) {
            ...
            if (results.data.length > 0) {
                ...
                currentTbody.innerHTML = `<tr><td>1</td><td>#</td><td>High</td><td>New Incident</td><td>Enter observation...</td><td>Open</td><td>TBD</td></tr>`;
            }
